...
<style>
  ...
  * { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  /* 상세 모달 — 모바일: 컴팩트 중앙 / PC: 이미지+정보 2단 와이드 */
  .modal { position:fixed; inset:0; z-index:100; background:rgba(0,0,0,.55);
           display:none; align-items:center; justify-content:center; padding:18px; }
  .modal.open { display:flex; }
  .sheet { background:#fff; width:100%; max-width:330px; max-height:80vh;
  ...
  .m-media { background:#f1f3f5; border-radius:16px 16px 0 0; overflow:hidden; }
  .m-img { width:100%; max-height:26vh; object-fit:contain; display:block; }
  ...
  @media(min-width:620px){
    .modal { padding:32px; }
    .sheet { max-width:780px; max-height:82vh; display:flex; overflow:hidden;
             border-radius:18px; }
    .m-media { flex:0 0 340px; border-radius:18px 0 0 18px; display:flex;
               align-items:center; justify-content:center; }
    .m-img { max-height:82vh; }
    .m-content { flex:1; min-width:0; overflow-y:auto; padding-bottom:20px; }
    .m-pad { padding:24px 24px 0; }
    .m-name { font-size:21px; margin:0 0 14px; padding-right:40px; }
    .m-badge { font-size:12px; margin-bottom:10px; }
    .m-dl { grid-template-columns:92px 1fr; gap:9px 14px; font-size:14.5px; }
    .m-memo { font-size:14px; line-height:1.65; margin:18px 0 0; }
    .m-btn { font-size:15px; padding:13px; margin:20px 24px 0; }
    .m-note { margin:12px 24px 0; }
  }
</style>
</head>
<body>
...
<div class="modal" id="modal">
  <div class="sheet">
    ...
    <div class="m-media"><img class="m-img" id="m-img" alt="" onerror="this.style.visibility='hidden'"></div>
    <div class="m-content">
      <div class="m-pad">
        <div id="m-badge"></div>
